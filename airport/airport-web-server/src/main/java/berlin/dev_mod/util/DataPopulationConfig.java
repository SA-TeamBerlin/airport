package berlin.dev_mod.util;

import java.util.Properties;

import berlin.config.ApplicationDomain;
import berlin.dbsetup.HibernateSetup;
import berlin.filter.NoDataFilter;
import berlin.ioc.ApplicationServerModule;
import berlin.serialisation.SerialisationClassProvider;
...
/**
 * Provides Berlin Airport Asset Management specific implementation of {@link IDomainDrivenTestCaseConfiguration} to be used for creation and population of the target development database from within of IDE.
 *
 * @author TG Team
 *
 */
public final class DataPopulationConfig implements IDomainDrivenTestCaseConfiguration {
    ...
    public DataPopulationConfig(final Properties props) {
    	...
    	try {
    	    ...
    	    props.setProperty("app.name", "Berlin Airport Asset Management");
    	    props.setProperty("reports.path", "");
    	    props.setProperty("domain.path", "../airport-pojo-bl/target/classes");
    	    props.setProperty("domain.package", "berlin");
    	    props.setProperty("tokens.path", "../airport-pojo-bl/target/classes");
    	    props.setProperty("tokens.package", "berlin.security.tokens");
    	    props.setProperty("workflow", "development");
    	    props.setProperty("email.smtp", "mail.berlin.com.ua");
    	    props.setProperty("email.fromAddress", "airport_support@berlin.com.ua");
